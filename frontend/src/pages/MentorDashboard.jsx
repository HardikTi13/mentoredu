    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Set your availability so admins can schedule mentoring sessions with you.</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <TabButton id="meetings"     label="My Sessions"  icon="📞" active={activeTab === "meetings"}     onClick={setActiveTab} />
        </div>
      </div>
      ...
      {activeTab === "availability" && (
        <div className="rounded-2xl border border-white/8 bg-white/3 backdrop-blur-sm animate-fade-in-up">
          <div className="p-4 border-b border-white/5 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="rounded-lg bg-slate-950 border border-slate-800 text-white text-sm px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              >
            ...
            <div className="flex items-center gap-3">
              ...
              {hasChanges && (
                <>
                  <button onClick={() => setToggles({})} className="px-3 py-1.5 rounded-lg border border-slate-600 text-slate-400 text-sm hover:bg-slate-800 transition">Cancel</button>
                  <button onClick={saveBatch} disabled={saving} className="px-4 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium transition disabled:opacity-50">
                    {saving ? "Saving…" : "Save"}
                  </button>
                </>
              )}
          ...
          {avError && <div className="mx-4 mt-4 p-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-300 text-sm">{avError}</div>}
                        ...
                        return (
                          <td key={dateStr} className="p-1">
                            <button
                              ...
                              disabled={disabled}
                              className={`w-full py-1.5 rounded-lg border font-medium text-xs tracking-wide transition-all duration-200
                                ${disabled ? "bg-slate-800/30 border-slate-800/50 cursor-not-allowed opacity-30 text-slate-600" : ""}
                                ${!disabled && enabled  ? "bg-emerald-600 border-emerald-500 text-white hover:bg-emerald-500 shadow-sm shadow-emerald-500/30" : ""}
                                ${!disabled && !enabled ? "bg-slate-800/50 border-slate-700/50 text-slate-600 hover:border-emerald-500/40 hover:bg-emerald-500/10 hover:text-slate-400" : ""}
                              `}
                            >
                              {enabled ? "✓" : "–"}
                            </button>
          ...
          <div className="px-4 py-3 border-t border-white/5 flex gap-6 text-xs text-slate-500">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-emerald-600" /> Available</div>
      ...
      {activeTab === "profile" && (
        <div className="max-w-2xl animate-fade-in-up">
          <div className="rounded-2xl border border-white/8 bg-white/3 backdrop-blur-sm p-6 space-y-5">
            <h2 className="text-lg font-semibold text-white">Mentor Profile</h2>
            <p className="text-slate-400 text-sm -mt-3">
              Your profile is managed by the admin. Tags and descriptions help match you to the right sessions.
            </p>

            {/* Tags (read-only for mentor) */}
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Your Tags</label>
              <div className="flex flex-wrap gap-2">
                {(user?.tags || []).length > 0 ? user.tags.map((tag) => (
                  <span key={tag} className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/15 text-emerald-300 border border-emerald-500/20">
                    {tag}
                  </span>
                )) : <span className="text-slate-600 text-xs italic">No tags assigned yet (managed by admin)</span>}
              </div>
            </div>
            ...
            <div>
              ...
              {user?.description ? (
                <p className="text-slate-300 text-sm leading-relaxed p-4 rounded-xl bg-white/3 border border-white/8">
                  {user.description}
                </p>
              ) : (
                <p className="text-slate-600 text-sm italic">No description yet (managed by admin)</p>
              )}
            </div>

            <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
              <p className="text-amber-300 text-xs">
                ℹ️ Your profile metadata (tags & description) is managed by the admin to ensure quality matching.
              </p>
            </div>
          </div>
        </div>
      ...
      {activeTab === "meetings" && (
        <div className="animate-fade-in-up">
          <div className="rounded-2xl border border-white/8 bg-white/3 backdrop-blur-sm p-6">
            <h2 className="text-lg font-semibold text-white mb-1">Your Mentoring Sessions</h2>
            <p className="text-slate-400 text-sm mb-6">Sessions scheduled for you by the admin.</p>
            ...
            {meetLoading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-slate-600 text-sm mt-1">Set your availability and wait for the admin to book sessions.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {meetings.map((m) => {
                  ...
                  return (
                    <div key={m.id} className="rounded-xl border border-white/8 bg-white/3 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-emerald-500/20 transition-all">
                      <div className="flex items-start gap-3">
                        <div className="text-2xl mt-0.5">{ct.icon}</div>
                        <div>
                          <h3 className="text-white font-semibold">{m.title}</h3>
                          <p className="text-slate-400 text-sm mt-0.5">
                            with <span className="text-emerald-400 font-medium">{m.user?.name}</span>
                          </p>
